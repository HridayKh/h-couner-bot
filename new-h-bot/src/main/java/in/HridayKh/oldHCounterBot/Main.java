...
public class Main {
		...
		while (true) {
			try {
				...
				if (currentTime < HttpUtil.nextRequestTime) {
					...
					System.out.println("Global API cooldown active. Sleeping for "
							+ sleepDuration / 1000 + " seconds.");
	...
	public void update_loop() {
		...
		if (TOKEN == null || token_exp == null || rn.isAfter(token_exp.minusMinutes(1))) {
			...
			// int expiresIn = newTokenArr != null ? Integer.parseInt(newTokenArr[1]) :
			// 3600;
			int expiresIn = 3000;
		...
		List<String[]> unreadMentions = MentionsManager
				.getUnreadMessagesAndFilterMentions(fullnamesToMarkAsRead);
		...
		for (String[] mention : unreadMentions) {
			...
			System.out.println("Processing Mention ID: " + id + ", Author: " + author + ", Target User: "
					+ targetUser);
			...
			try {
				...
				System.err.println("An error occurred during processing mention ID " + id + ": "
						+ e.getMessage());
				e.printStackTrace();
		...
		if (!fullnamesToMarkAsRead.isEmpty()) {
			System.out.println("Attempting to batch mark " + fullnamesToMarkAsRead.size()
					+ " messages as read.");
			...
			if (batchMarked) {
				...
				saveProcessedMessageFullnames(); // Persist the updated set after successful batch
									// marking
			} else {
				System.err.println(
	...
	private void loadProcessedMessageFullnames() {
		...
		try {
			if (!file.exists()) {
				System.out.println("Processed IDs file not found. Attempting to create: "
						+ PROCESSED_IDS_FILE);
				if (file.getParentFile() != null && !file.getParentFile().exists()) {
					file.getParentFile().mkdirs(); // Create parent directories
				}
				if (file.createNewFile()) {
					System.out.println("Successfully created new processed IDs file: "
							+ PROCESSED_IDS_FILE);
				} else {
					System.err.println("Failed to create new processed IDs file: "
							+ PROCESSED_IDS_FILE);
					return;
				}
			}
			...
			try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
				...
				System.out.println("Loaded " + processedMessageFullnames.size()
						+ " processed message fullnames from "
						+ PROCESSED_IDS_FILE);
			}
		} catch (IOException e) {
	...
	private void saveProcessedMessageFullnames() {
		if (PROCESSED_IDS_FILE == null || PROCESSED_IDS_FILE.isEmpty()) {
			System.err.println(
					"PROCESSED_IDS_FILE environment variable not set. Cannot save processed messages.");
			return;
		}
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(PROCESSED_IDS_FILE))) {
			...
			System.out.println(
					"Saved " + processedMessageFullnames.size() + " processed message fullnames to "
							+ PROCESSED_IDS_FILE);
		} catch (IOException e) {
			System.err.println(
					"Could not save processed message fullnames to file " + PROCESSED_IDS_FILE
							+ ": " + e.getMessage());
			e.printStackTrace();
		}
